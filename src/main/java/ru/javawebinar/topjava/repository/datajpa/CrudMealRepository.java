...
public interface CrudMealRepository extends JpaRepository<Meal, Integer> {
    ...
    @Query("SELECT m FROM Meal m WHERE m.user.id=:userId AND m.dateTime " +
            "BETWEEN :from AND :to ORDER BY m.dateTime DESC")
    List<Meal> getBetween(@Param("userId") int userId,
                          @Param("from") LocalDateTime from,
                          @Param("to") LocalDateTime to);

    @Query("SELECT m FROM Meal m LEFT JOIN FETCH m.user WHERE m.id=:mealId")
    Meal getOneById(@Param("mealId") int mealId);
}
